...
create table Station(ID int,CITY CHAR(20),STATE CHAR(2),LAT_N int,LONG_W int,PRIMARY KEY(ID));/*The CREATE TABLE statement is used to create a new table in a database.*/
describe Station;
-----------------------------
...
select * from  Station; /*The SELECT statement is used to select data from a database.*/
-----------------------------------------------------
/* 4. Execute a query to select Northern stations (Northern latitude > 39.7).*/
select * from  Station  where LAT_N > 39.7;/*The WHERE clause is used to filter records*/
------------------------------------------------------
...
select St.TEMP_F,Sn.CITY from Station Sn RIGHT JOIN STATS St ON Sn.id=St.id;/* RIGHT JOIN keyword returns all records from the right table (table2), and the matching records from the left table (table1). The result is 0 records from the left side, if there is no match*/
--------------------------------------------
...
group by Sn.CITY; /*The MIN() function returns the smallest value of the selected column ,The MAX() function returns the largest value of the selected column and The SUM() function returns the total sum of a numeric column*/
