...
update student set AGE=timestampdiff(Year,New_dob,Current_Date()) 

select timestampdiff(Year,New_dob,Current_Date()) as Age_In_Years from student;
...
/*5 b) 	b) 	List the students who are common members of the Chemistry Club and Biology Club but not of the Physics Club.*//*(i think so this not the right answer*/

SELECT FullName
FROM Chem
WHERE Chem.fullname NOT IN (SELECT fullname FROM Phy)
