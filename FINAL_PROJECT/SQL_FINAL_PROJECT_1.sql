...
select * from Station Sn RIGHT JOIN STATS St ON Sn.id=St.id group by Sn.CITY /* STEP 2: THINNKING DEEP INTO IT MORE*/
...
group by Sn.CITY /* STEP 3: Probably the Answer*/
---------------------------------------------------------------
/*11.Execute a query to display each city’s monthly temperature in Celcius and rainfall in
Centimeter.*/
select * from Station Sn RIGHT JOIN STATS St ON Sn.id=St.id group by St.MONTH,Sn.CITY  
select Sn.CITY,TEMP_F,((St.TEMP_F* 9/5) + 32) as TEMP_C,(St.RAIN_I/0.39370) as RAIN_CM,St.RAIN_I 
from Station Sn RIGHT JOIN STATS St ON Sn.id=St.id group by St.MONTH,Sn.CITY  /* STEP 3: Probably the Answer*/
---------------------------------------------------------------
